...
struct EyeInfo findEyeCenter(cv::Mat face, cv::Rect eye, std::string debugWindow, cv::CascadeClassifier &eye_cascade) {
	cv::Mat eyeROIUnscaled = face(eye);
	cv::Mat eyeROI;
	std::vector<cv::Rect> eyes;
	struct EyeInfo eyeInfo;
	scaleToFastSize(eyeROIUnscaled, eyeROI);

	eye_cascade.detectMultiScale(eyeROI, eyes, 1.1, 2, 0 | CV_HAAR_SCALE_IMAGE, cv::Size(30, 30), cv::Size(50, 50));

	//for (int i = 0; i < eyes.size(); i++)
		//imshow(debugWindow, eyeROI(eyes[0]));
